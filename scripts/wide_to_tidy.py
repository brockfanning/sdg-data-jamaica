...
def tidy_dataframe(df, indicator_variable, csv):
    """This converts the data from wide to tidy, based on the column names."""

    debug = False
    if csv == 'data/indicator_4-1-1.csvREMOVEME':
        # Use this variable for random debug of particular indicators if needed.
        debug = True
    ...
    for column in columns:
        if column == indicator_variable:
            # If this is the indicator variable, use this as the main
            # column.
...
def tidy_csv(csv, subnational_folders):
    ...
    # If this dataframe already has columns for the "tidy" year and value,
    # assume it has already been converted.
    if HEADER_YEAR_TIDY in df.columns and HEADER_VALUE_TIDY in df.columns:
        #print('Skipping ' + csv + ' - already converted.')
        return True

    try:
        tidy = tidy_dataframe(df, metadata['indicator_variable'], csv)
    except Exception as e:
        print(csv, e)
        return False
